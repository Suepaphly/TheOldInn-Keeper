...
module.exports.run = async (client, message, args) => {
 ...
 let timeoutarray = [dailytimeout, fishtimeout, crafttimeout, gathertimeout, hunttimeout, worktimeout, robtimeout, deposittimeout];
 let namearray = ["Daily", "Fish", "Craft", "Gather", "Hunt", "Work", "Rob", "Deposit"];
 ...
 var cooldownEmbed = new Discord.MessageEmbed()
  ...
  for (let i = 0; i < timeoutarray.length; i++) {
     if (userarray[i] !== null && timeoutarray[i] - (Date.now() - userarray[i]) > 9000) {
         let time = ms(timeoutarray[i] - (Date.now() - userarray[i]));
         
         cooldownEmbed.addField( namearray[i], ` => ${time.hours}h, ${time.minutes}m, ${time.seconds}s.`); 
