...
module.exports.run = async (client, message, args) => {
    try {
        ...
        for (let location of locations) {
            ...
            const availableSlots = wallCount / 10;

            // Get army info
            const armyData = (await db.get(`Troops_${location}`)) || {};
            const armyEntries = Object.entries(armyData).filter(
                (entry) => entry[0] !== "total",
            );
            ...
            const trapEntries = Object.entries(trapData).filter(
                (entry) => entry[0] !== "total",
            );
        ...
        message.channel.send(
            "There was an error displaying the map. Please try again later.",
        );
    }
};
