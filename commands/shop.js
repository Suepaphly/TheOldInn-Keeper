    ...
    if (args[0] === "buy" && args[1]) {
        ...
        if (weapons[item]) {
            if (money < weapons[item].cost) {
                return message.channel.send(`❌ You need ${weapons[item].cost.toLocaleString()} kopeks to buy a ${weapons[item].name}. You have ${money.toLocaleString()}.`);
            }
            ...
            if (money < armor[item].cost) {
                return message.channel.send(`❌ You need ${armor[item].cost.toLocaleString()} kopeks to buy ${armor[item].name}. You have ${money.toLocaleString()}.`);
            }
            ...
            message.channel.send(`❌ Item not found! Available items: knife, sword, pistol, shotgun, rifle, cloth, leather, chainmail, studded, plate`);
        }
    } else {
        message.channel.send(`❌ Use \`=shop\` to view items or \`=shop buy [item]\` to purchase.`);
    }
};
