...
async function startBettingRound() {
    const embed = new EmbedBuilder()
        ...
        .setColor("#FF6B6B")
        .setDescription("**Betting is now open!** Use `=rbet [type] [amount]` to place your bet.\n\n⏰ **15 seconds** to place bets...")
        .addFields({ name: "Current Bets", value: global.rouletteGame.bets.length === 0 ? "No bets placed yet" : global.rouletteGame.bets.map(bet => `<@${bet.userId}>: ${bet.type} - ${bet.amount} kopeks`).join('\n'), inline: false })
...
async function lastCallForBets() {
    const embed = new EmbedBuilder()
        ...
        .setColor("#FFA500")
        .setDescription("**Last call for bets!** This is your final chance.\n\n⏰ **15 seconds** remaining...")
...
async function spinWheel() {
    ...
    setTimeout(async () => {
        ...
        if (global.rouletteGame.lastNumbers.length > 10) {
            global.rouletteGame.lastNumbers = global.rouletteGame.lastNumbers.slice(0, 10);
        }
        ...
        for (const bet of global.rouletteGame.bets) {
            const multiplier = calculatePayout(bet.type, bet.amount, winningNumber);
            const winnings = bet.amount * multiplier;
            
            if (multiplier > 0) {
                await db.add(`money_${bet.userId}`, winnings);
                results.push(`✅ <@${bet.userId}>: Won ${winnings} kopeks (${bet.type})`);
                totalWinnings += winnings;
            } else {
                results.push(`❌ <@${bet.userId}>: Lost ${bet.amount} kopeks (${bet.type})`);
            }
        }
        ...
        const resultEmbed = new EmbedBuilder()
            ...
            .setDescription(`**The ball landed on:** ${colorEmoji} **${winningNumber}** (${color})`)
            .addFields(
                { name: "Results", value: results.length > 0 ? results.join('\n') : "No bets were placed", inline: false },
                { name: "Total Winnings", value: `${totalWinnings} kopeks`, inline: true },
                { name: "Total Bets", value: `${global.rouletteGame.bets.length}`, inline: true }
            )
            .setFooter({ text: "Thank you for playing! Use =startroulette to play again." });
...
module.exports.run = async (client, message, args) => {
    ...
    if (!command || command === 'start') {
        ...
        const startEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: "How to Play", value: "Use `=rbet [type] [amount]` to place your bet\nExample: `=rbet red 100`", inline: false },
                { name: "Available Bets", value: "Use `=rhelp` for a full list of betting options", inline: false }
            )
