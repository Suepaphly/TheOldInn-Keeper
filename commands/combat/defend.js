...
module.exports.run = async (client, message, args) => {
    ...
    if (hasAttackedThisTurn) {
        return message.channel.send("⏰ You have already defended this turn! Wait for the next turn.");
    }
    ...
    if (killedMonster) {
        ...
        if (rewardWinner === user.id) {
            message.channel.send(`🛡️ ${member} defends the town and slays a ${killedMonster} with ${damageDealt} damage, claiming ${reward} kopeks! ${remainingMonsters} monsters remaining.`);
        } else {
    ...
    if (!killedMonster) {
        if (hitMonster) {
            ...
            message.channel.send(`🛡️ ${member} defends the town and hits a ${hitMonster} for ${damageDealt} damage! (${currentDamage}/${maxHealth} damage) ${remainingMonsters} monsters remaining.`);
        } else {
            // Get updated monster count for display
            const updatedMonsters = await db.get("Monsters") || {};
            const remainingMonsters = Object.values(updatedMonsters).reduce((sum, count) => sum + count, 0);
            
            message.channel.send(`🛡️ ${member} defends the town but misses! ${remainingMonsters} monsters remaining.`);
        }
    }
};
