...
module.exports.run = async (client, message, args) => {
    ...
    // Check if target user is actually defeated
    const defeatTimer = await db.get(`defeated_${targetUser.id}`);
    
    if (!defeatTimer) {
        return message.channel.send(`❌ ${targetUser.user.username} is not currently defeated and doesn't need to be revived.`);
    }

    // Check if defeat timer is still active (24 hours = 86400000 ms)
    const timeRemaining = 86400000 - (Date.now() - defeatTimer);
    
    if (timeRemaining <= 0) {
        // Timer already expired, clean up the database
        await db.delete(`defeated_${targetUser.id}`);
